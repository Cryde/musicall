<template>
  ...
  <div class="flex flex-row gap-5">
    ...
    <div class="basis-4/12">
      ...
      <div class="flex flex-wrap justify-end-safe gap-4 mb-10">
        <Button label="Poster une annonce" icon="pi pi-plus" severity="info" size="small"/>
      </div>
        ...
        <Card
          ...
          <template #content>
            <template v-if="isTypeBand(announce.type)">
              {{ announce.author.username }} est un <strong>{{ announce.instrument.musician_name.toLocaleLowerCase() }}</strong> et cherche un groupe jouant du
              <strong>{{
            ...
            <template v-if="isTypeMusician(announce.type)">
              {{ announce.author.username }} cherche pour son groupe un <strong>{{ announce.instrument.musician_name.toLocaleLowerCase() }}</strong> jouant du
              <strong>{{
                  announce.styles.map(style => style.name.toLocaleLowerCase()).join(', ')
...
musicianAnnounceStore.loadLastAnnounces();
