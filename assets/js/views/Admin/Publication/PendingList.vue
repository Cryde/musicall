<template>
  <div class="flex flex-col gap-6">
    ...
    <DataTable
      ...
      :loading="adminPublicationStore.isLoading"
      stripedRows
      tableStyle="min-width: 50rem"
    >
      <template #empty>
        <div class="text-center py-8 text-surface-500">
          Il n'y a pas de publications en attente de validation.
        </div>
      </template>

      <Column field="title" header="Titre" sortable />

      <Column field="sub_category.title" header="Catégorie" sortable />
...
onMounted(async () => {
  await adminPublicationStore.loadPendingPublications()
})

function confirmApprove(publication) {
  confirm.require({
    message: `Valider la publication "${publication.title}" ?`,
    header: 'Confirmation',
    ...
    accept: async () => {
      try {
        await adminPublicationStore.approvePublication(publication.id)
        await notificationStore.loadNotifications()
        toast.add({
          severity: 'success',
          summary: 'Publication validée',
          detail: `La publication "${publication.title}" a été validée.`,
          life: 3000
        })
      } catch (e) {
        toast.add({
...
function confirmReject(publication) {
  confirm.require({
    message: `Rejeter la publication "${publication.title}" ?`,
    header: 'Confirmation',
    ...
    accept: async () => {
      try {
        await adminPublicationStore.rejectPublication(publication.id)
        await notificationStore.loadNotifications()
        toast.add({
          severity: 'info',
          summary: 'Publication rejetée',
          detail: `La publication "${publication.title}" a été rejetée.`,
          life: 3000
        })
      } catch (e) {
        toast.add({
